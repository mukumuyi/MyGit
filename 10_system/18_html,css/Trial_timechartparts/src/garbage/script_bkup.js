...
(function () {
  d3.timeline = function() {
    ...
    var hover = function () {},
        ...
        orient = "bottom",width = null,height = null,
        ...
        axisBgColor = "white",
        // 軸の設定
        tickFormat = { //format: d3.time.format("%Y/%m/%d %H:%M"),
    ...
    var tickformatDT = function(date) {
      ...
      return date.getHours()%(frametimespan/3600000) === 0  ? formatDateAndTime(date) : formatTime(date);
    };
    ...
    var appendBackgroundBar = function (yAxisMapping, index, g, data, datum) {
      ...
      itemHeight * d.lane + itemMargin * d.group;
    ...
    var appendLabel = function (gParent,  index, hasLabel, datum,d,i) {
      ...
      var rowsDown          = margin.top + (fullItemHeight/2) + itemHeight * datum.lane + itemMargin * datum.group ;
      gParent.append("text")
    ...
    function timeline (gParent) {
      ...
      if (stacked || ending === 0 || beginning === 0) {
        g.each(function (datas) {
          datas.forEach(function (data) {
            // console.log(datum)
              maxgroup = data.group + 1;
              maxlane = data.lane + 1;
              tempname = data.name ;
              if(beginning === 0)
                if (data.starting_time < minTime || (minTime === 0 && timeIsRelative === false))
                  minTime = data.starting_time;
              if(ending === 0)
                if (data.ending_time > maxTime)
                  maxTime = data.ending_time;
      ...
      g.each(function(d, i) {
        ...
        d.forEach( function(datum, index){
          ...
          g.selectAll("svg").data(data).enter()
            ...
            .on("mouseover", function(d,i) {
              ...
              tooltip
                ...
                .html("name : " + datum.name
                 + "<br>start : " + starting_time.toLocaleDateString()
                 + " " + starting_time.toLocaleTimeString('it-IT')
                 + "<br>end : " + ending_time.toLocaleDateString()
                 + " " + ending_time.toLocaleTimeString('it-IT')
                 + "<br>description : " + maxlane    
                 + "<br>description2 : " 
                  );
            })
            ...
            .attr("class", function (d, i) {
              return data.class ? "timelineSeries_"+data.class : "timelineSeries_"+index;
            })
          ...
          if (hasLabel) { appendLabel(gParent, index, hasLabel, datum,d,i); }

          function getStackPosition(d, i) {
            if (stacked) {
              return margin.top + itemHeight * d.lane + itemMargin * d.group; // Add By muku
            }
            return margin.top;
          }
          function getStackTextPosition(d, i) {
            if (stacked) {
              return margin.top + itemHeight * d.lane + itemMargin * d.group + itemHeight * 0.75; // Add By muku
            }
      ...
      var belowLastItem = (margin.top +  itemHeight * maxlane + itemMargin * maxgroup);
...
function timeline(frametimespan) {
  ...
  // JSONデータを読み込む関数
    ...
    request.onload = function() {
      ...
      // .datum(trialdata).call(chart);
      .datum(csvArray).call(chart);
...
const jsonFilePath = 'http://127.0.0.1:5500/data/trialdata.json';
