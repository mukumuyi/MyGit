import React, { useState,Dispatch, useRef, useEffect, memo, FormEvent } from "react";

import Rect from "../atoms/Rect";
import Line from "../atoms/Line";
import ControlPanel from "../organism/ContolPanel";
import { DrawNewProperty } from "./DataInput";
...
interface PropsType  {
  ...
  colSelector: ColorOption[];
  convDef: ConvDefType;
  inputData: InputDataType[];
  minStart: number;
  originData: InputDataType[];
  setColorSelected: Dispatch<React.SetStateAction<ColorOption[]>>;
  setConvDef: Dispatch<React.SetStateAction<ConvDefType>>;
  setInputData: Dispatch<React.SetStateAction<InputDataType[]>>;
};

export const PlotArea = memo((props:PropsType) => {
  // props
  const {
    colorSelected,
    colSelector,
  ...
  const [timeSelected, setTimeSelected] = useState<number>(BasicProperty.timeSeleced);
  const [widthSelected, setWidthSelected] = useState<number>(
  ...
  const onChangeCol = (e) => {
    ...
    if (name === "colColor") {
      const uniqueStatusList = Array.from (new Set(inputData.map((item) => item[value])));
  ...
  let mouseOverTimer: number | NodeJS.Timeout;
  ...
  const minTimeStamp = minStart
    ...
    : Math.floor(
        Math.min(
          ...inputData
            .map((item) => Number(item.starting_time))
  ...
  const plotArray = inputData
    .filter(
      (item) =>
        ...
        gMargin * (item.group - 1 / 2) +
          widthSelected * (item.lane - 1) +
          cordinate.y <
          svgSize.height - plotStartY &&
        gMargin * (item.group - 1 / 2) +
          widthSelected * item.lane +
          cordinate.y >
          0,
    )
    .map((item) => ({
      ...
      x:
        ...
        cordinate.x,
      y:
        gMargin * (Number(item.group) - 1 / 2) +
        widthSelected * (Number(item.lane) - 1) +
        plotStartY +
        cordinate.y,
      fill:
  ...
  const maxTimeStamp =
    Math.ceil(
      Math.max(
        ...plotArray
          .map((item) => Number(item.ending_time))
          .filter((value) => !isNaN(value)),
      ) / 3600000,
    ) * 3600000;
  ...
  const xAxisArray = Array.from(
    ...
    (_, index) => {
      ...
      return {
        ...
        coord: "xAxis",
        cx:
          (Number(currentTime) - minTimeStamp) * scaleFactor + plotStartX + cordinate.x,
        cy: 25,
        labelD: `${dates}`,
        labelT: `${hours}`,
